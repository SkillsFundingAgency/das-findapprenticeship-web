...
namespace SFA.DAS.FAA.Application.UnitTests.Queries.Vacancies
{
    [TestFixture]
    public class WhenHandlingGetNhsApprenticeshipVacancyQuery
    {
        [Test, MoqAutoData]
        public async Task Then_Result_Is_Returned(
           GetNhsApprenticeshipVacancyQuery query,
           GetNhsApprenticeshipVacancyApiResponse expectedResponse,
           [Frozen] Mock<IApiClient> apiClient,
           GetNhsApprenticeshipVacancyQueryHandler handler)
        {
            // Mock the response from the API client
            var expectedGetUrl = new GetNhsApprenticeshipVacancyApiRequest(query.VacancyReference);
            apiClient.Setup(client => client.Get<GetNhsApprenticeshipVacancyApiResponse>(It.Is<GetNhsApprenticeshipVacancyApiRequest>(c => c.GetUrl.Equals(expectedGetUrl.GetUrl))))
        ...
        [Test, MoqAutoData]
        public void Then_Result_Is_Null_Not_Found_Exception_Returned(
           GetNhsApprenticeshipVacancyQuery query,
           [Frozen] Mock<IApiClient> apiClient,
           GetNhsApprenticeshipVacancyQueryHandler handler)
        {
            ...
            var expectedGetUrl = new GetNhsApprenticeshipVacancyApiRequest(query.VacancyReference);
            apiClient.Setup(client => client.Get<GetNhsApprenticeshipVacancyApiResponse>(It.Is<GetNhsApprenticeshipVacancyApiRequest>(c => c.GetUrl.Equals(expectedGetUrl.GetUrl))))
                .ReturnsAsync((GetNhsApprenticeshipVacancyApiResponse)null!);

            // Assert
            Assert.ThrowsAsync<ResourceNotFoundException>(() => handler.Handle(query, CancellationToken.None));
        }
    }
}
