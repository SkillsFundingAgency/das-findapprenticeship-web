    ...
    public class WhenDeletingSavedVacancyCommand
    {
            ...
            [Greedy] DeleteSavedVacancyCommandHandler handler)
        {
            var expectedApiRequest =
                new PostDeleteSavedVacancyApiRequest(command.CandidateId, new PostDeleteSavedVacancyApiRequestData { VacancyReference = command.VacancyReference });

            apiClient.Setup(x =>
                    x.PostWithResponseCode(
                        It.Is<PostDeleteSavedVacancyApiRequest>(r => r.PostUrl == expectedApiRequest.PostUrl
                                                                     && ((PostDeleteSavedVacancyApiRequestData)r.Data).VacancyReference == command.VacancyReference
                        )));
