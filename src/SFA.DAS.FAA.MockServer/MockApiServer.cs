...
public static class MockApiServer
{
    public static IWireMockServer Start()
    {
        ...
        server.Given(Request.Create().WithPath(s => Regex.IsMatch(s, "/candidates", RegexOptions.None, regexMaxTimeOut))
            .UsingPut())
        .RespondWith(
            Response.Create()
                .WithStatusCode(200)
        ...
        server.Given(Request.Create().WithPath(s => Regex.IsMatch(s, "/jobs/summary", RegexOptions.None, regexMaxTimeOut))
                .UsingGet())
            .RespondWith(
                Response.Create()
                    .WithStatusCode(200)
                    .WithHeader("Content-Type", "application/json")
                    .WithBodyFromFile("candidate-workhistory-summary.json"));
