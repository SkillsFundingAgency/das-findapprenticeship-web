...
public static class MockApiServer
{
    public static IWireMockServer Start()
    {
        ...
        server.Given(Request.Create().WithPath(s => Regex.IsMatch(s, "/vacancies/\\d+$"))
                .UsingGet())
            .RespondWith(
                Response.Create()
                    .WithStatusCode(200)
        ...
        server.Given(Request.Create().WithPath(s => Regex.IsMatch(s, "/candidates/\\S+$"))
                 .UsingPut())
              .RespondWith(
                 Response.Create()
                     .WithStatusCode(202)
        ...
        server.Given(Request.Create().WithPath(s => Regex.IsMatch(s, "/users/\\S+/add-details"))
                .UsingPut())
            .RespondWith(
                Response.Create()
                    .WithStatusCode(202)
