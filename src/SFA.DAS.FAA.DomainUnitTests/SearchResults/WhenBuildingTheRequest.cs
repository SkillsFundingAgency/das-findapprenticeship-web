...
public class WhenBuildingTheRequest
{
    ...
    public void Then_The_Url_Is_Correctly_Constructed(string location, List<string> routes, List<string> levels, int distance, string searchTerm, int pageNumber, int pageSize, VacancySort sort, bool disabilityConfident)
    {
        ...
        actual.GetUrl.Should().Be($"searchapprenticeships/searchResults?location={location}&routeIds={string.Join("&routeIds=",routes)}&distance={distance}&searchTerm={searchTerm}&pageNumber={pageNumber}&pageSize={pageSize}&sort={sort}&levelIds={string.Join("&levelIds=", levels)}&disabilityConfident={disabilityConfident}");
    }
    ...
    public void Then_With_No_Params_The_Url_Is_Correctly_Built()
    {
        ...
        actual.GetUrl.Should().Be("searchapprenticeships/searchResults?location=&routeIds=&distance=&searchTerm=&pageNumber=&pageSize=&sort=DistanceAsc&levelIds=&disabilityConfident=False");
    }
}