...
public class WhenCallingGetEmploymentLocations
{
        ...
        [Frozen] Mock<IMediator> mediator)
    {
        foreach (var address in queryResult.EmploymentLocation.Addresses)
        {
            ...
            address.FullAddress = JsonConvert.SerializeObject(mockAddress);
        }
    ...
    [Test, MoqAutoData]
    public async Task Then_Empty_Then_View_Is_Returned(
        Address mockAddress,
        Guid applicationId,
        Guid candidateId,
        GetEmploymentLocationsQueryResult queryResult,
        [Frozen] Mock<IMediator> mediator)
    {
        ...
        {
            address.IsSelected = false;
            address.FullAddress = JsonConvert.SerializeObject(new Address("", "", "", "", ""));
        }
        var mockUrlHelper = new Mock<IUrlHelper>();
        mockUrlHelper
            .Setup(x => x.RouteUrl(It.IsAny<UrlRouteContext>()))
