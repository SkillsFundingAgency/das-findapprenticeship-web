...
{
    [Test, MoqAutoData]
    public void Map_Returns_Expected_Result(GetDeleteVolunteeringOrWorkExperienceQueryResult source)
    {
        ...
        using (new AssertionScope())
        {
            ...
            result.Dates.Should().Be($"{source.FromDate:MMMM yyyy} to {source.ToDate:MMMM yyyy}");
        }
    }

    [Test, MoqAutoData]
    public void When_EndDate_Is_Null_Map_Returns_Expected_Result(GetDeleteVolunteeringOrWorkExperienceQueryResult source)
    {
        source.ToDate = null;
        var result = (DeleteVolunteeringOrWorkExperienceViewModel)source;
        ...
        using (new AssertionScope())
        {
            ...
            result.Description.Should().Be(source.Description);
            result.ApplicationId.Should().Be(source.ApplicationId);
            result.Dates.Should().Be($"{source.FromDate:MMMM yyyy} onwards");
        }
    }
}
