...
if ($keywordsInput.length > 0) {
  $keywordsInput.wrap(
    '<div id="autocomplete-container" class="das-autocomplete-wrap"></div>'
  );
...
var removeFormDone = function (data, form) {
  ...
  removeForm.attr(
    "action",
    "/shortlist/items/00000000-0000-0000-0000-000000000000"
  );
...
var updateShortlistCount = function (remove) {
  ...
  shortlistCountsUi
    .text(currentCount)
    .addClass("app-view-shortlist-link__number-update");
...
FeedbackGraph.prototype.legendHtml = function (row) {
  ...
  nodeListForEach(dataCells, function (dataCell) {
    if (isNaN(dataCell.textContent)) {
      ...
      legendListItem.className =
        "app-graph-key__list-item app-graph-key__list-item--colour-" +
        (cellCount + 1);
...
FeedbackGraph.prototype.barHtml = function (dataCell, barCount) {
  ...
  span2.className =
    "app-graph__bar-value app-graph__bar-value--colour-" + barCount;
...
ShowHidePanels.prototype.showHideButton = function (item) {
  ...
  button.className =
    "govuk-button govuk-button--secondary govuk-!-font-size-16";
